...
from modules.loss import SoftTarget
...
def parse_agrs():
    ...
    parser.add_argument('--ann_path', type=str, default='data/mimic_cxr/mimic_annotation_mrg.json', help='the path to the directory containing the data.')
    ...
    parser.add_argument('--save_dir', type=str, default='results/mrg', help='the path to save the models.')
...
def main():
    ...
    tmodel = blip_decoder(args, tokenizer, image_size=args.image_size, prompt=prompt_temp)
    tcheckpoint = torch.load("./results/mrg/model_best_t_gk4.24.pth", map_location='cpu')
    msg = tmodel.load_state_dict(tcheckpoint, strict=False)
    ...
    trainer = Trainer(model, tmodel, criterionKD, criterion_cls, base_probs, metrics, args, train_dataloader, val_dataloader, test_dataloader, device, utils.is_main_process)
    trainer.train()
